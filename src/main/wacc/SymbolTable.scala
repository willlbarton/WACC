...
case class SymbolTable(parent: Option[SymbolTable]) {
  private val table: mutable.Map[Var, Identifier] = mutable.HashMap()

  def update(ident: Var, obj: Identifier): Unit = table += ident -> obj
  
  // Checks for the identifier in the current scope and all parent scopes
  def apply(ident: Var): Option[Identifier] =
  ...
  def contains(ident: Var): Boolean = table.contains(ident)
